...
export default function GrammarList({ lessons, level, settings, favorites, toggleFavorite }) {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="border border-ai-line dark:border-night-line rounded-lg overflow-hidden bg-paper dark:bg-night-paper divide-y divide-ai-line dark:divide-night-line">
        {visible.map((p) => {
          ...
          return (
            <div key={p.id}>
              <div className="flex items-center gap-2 px-3 py-2.5 hover:bg-washi dark:hover:bg-night transition-colors">
                <button
                  ...
                  {p.examples.length > 0 && (
                    <div className="bg-sakura-soft dark:bg-night-paper rounded-md p-3 space-y-2.5">
                      {p.examples.map((ex, ei) => (
                        <div key={ei}>
                          <div className="font-mincho text-base text-ink dark:text-night-ink">{ex.jp}</div>
                          {ex.meaningBn && (
