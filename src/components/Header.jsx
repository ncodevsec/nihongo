...
function SettingsIcon({ className = "w-4 h-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={`${className} shrink-0`} aria-hidden="true">
...
function BackIcon({ className = "w-4 h-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className={`${className} shrink-0`} aria-hidden="true">
      <polyline points="15 18 9 12 15 6" />
    </svg>
  );
}

export default function Header({ active, onChange, onBack, moduleKey, accuracy, masteredCount, total, settings }) {
      ...
      <div className="relative max-w-4xl mx-auto px-3 sm:px-5 py-3 sm:py-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            {!isGrammar && !isSettings && (
              <div className="hidden sm:flex items-center gap-4 font-mono text-xs">
                <div className="text-right">
                  <div className="text-shu dark:text-shu-glow font-semibold">{accuracy}%</div>
            ...
            <button
              onClick={() => (isSettings ? onBack() : onChange("settings"))}
              aria-label={isSettings ? T("backButton") : T("tabSettings")}
              title={isSettings ? T("backButton") : T("tabSettings")}
