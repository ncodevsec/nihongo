...
function buildQuestions(pool, allData, { isVocab, vocabLang, optionCount }) {
	const textOf = (d) =>
		isVocab && vocabLang === "en" ? d.meaningEn || d.meaning : d.meaning;
...
export default function Quiz({
	...
	const [setupOptionCount, setSetupOptionCount] = useState(
		settings.quizOptionCount,
	);
	...
	const setupAvailableCount =
		setupLength === "all"
			? setupCategoryFiltered.length
			: Math.min(Number(setupLength), setupCategoryFiltered.length);
	...
	const startQuizRun = useCallback(
		({ cat, len, opts, timed, minutes, nextMode, source }) => {
			...
			const buildOpts = {
				isVocab,
				vocabLang: settings.vocabLang,
				optionCount: opts,
			};
			setQuestions(
				applyLength(
					shuffle(buildQuestions(source, kanjiData, buildOpts)),
					len,
				),
			);
			setCurrent(0);
					...
					<div className="space-y-4">
							...
							<div className="flex flex-wrap gap-2">
								{LENGTH_OPTIONS.map((len) => (
									<button
										...
										className={`px-3 py-1.5 rounded-md border text-sm font-bengali ${
											setupLength === len
							...
							<div className="flex gap-2">
								{[2, 3, 4, 5].map((n) => (
									<button
										...
										className={`w-10 h-10 rounded-md border text-sm font-mono font-semibold ${
											setupOptionCount === n
						...
						<div className="flex items-center justify-between">
							...
							<button
								...
								className={`w-11 h-6 shrink-0 rounded-full relative ${
									setupTimed
										? "bg-ai dark:bg-ai-glow"
	...
	const restartButton = (
		<button
			...
			title={T("quizRestart")}
			className="w-8 h-8 shrink-0 flex items-center justify-center rounded-md border border-ai-line dark:border-night-line bg-paper dark:bg-night-paper text-shu dark:text-shu-glow hover:bg-shu-soft dark:hover:bg-shu/10"
		>
			<svg
				viewBox="0 0 24 24"
				className="w-4 h-4"
				...
				<div className="bg-paper dark:bg-night-paper border border-ai-line dark:border-night-line rounded-lg shadow-card dark:shadow-none p-6">
					...
					<div className="grid grid-cols-2 gap-2 mb-2">
						<button
							...
							className="font-bengali text-sm border border-ai-line dark:border-night-line text-ai dark:text-ai-glow rounded-md py-2 hover:bg-ai-soft dark:hover:bg-night-line"
						>
							{T("quizAll")}
						</button>
						<button
							...
							className="font-bengali text-sm border border-shu/40 dark:border-shu-glow/40 text-shu dark:text-shu-glow rounded-md py-2 hover:bg-shu-soft dark:hover:bg-shu/10 disabled:opacity-30 disabled:cursor-not-allowed"
						>
					...
					<button
						onClick={goToSetup}
						className="w-full font-bengali text-xs text-ink-muted dark:text-night-ink-muted hover:text-shu dark:hover:text-shu-glow py-2"
					>
						{T("quizSetupTitle")}
					</button>
				</div>
					...
					{q.options.map((opt, idx) => {
						...
						return (
							<button
								...
								disabled={answered}
								className={`w-full text-left border rounded-md px-3 py-2 ${style} ${
									!answered
										? "cursor-pointer"
										: "cursor-default"
								}`}
